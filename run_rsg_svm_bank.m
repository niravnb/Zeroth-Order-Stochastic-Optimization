...
NList = [10, 100, 250, 500, 1000, 1850, 3000, 5000]; % Iterations list
T = 50; % Replications

%% Loading Dataset

load('bank_dataset.mat');
...
d = size(u,2); % Dimension of the problem
dataset_len = size(u,1); % Dataset length
reduce_dim = 0; % Reduce dimension by SVD?

x_1 = initial.*unifrnd(-1,1,d,1); % Initial point of d dimension
...
training_len = round(0.6*dataset_len);
test_len = round(0.4*dataset_len);
...
L = max(hessian_norm); % Estimate of L
...
alpha_1 = (2 + lambda*d*3^2)^2; % Estimate of E[f(x + eta*delta)^2] upper bound
...
rdsa_kw = [NList', zeros(length(NList),4)];
...
for N = NList
    ...
    ub(i,2:5) = rsg_svm(algo, N, T, approx_samples, x_1, gamma, P_R, eta, lambda, training_u, training_v, test_u, test_v);
    i = i+1;
end
disp(ub(:,[2,4]));
disp('--------------------------------End Unbiased----------------------------------');
...
plot(NList,ub(:,2));
save(['svm_bank_ub_step_',num2str(step),'_x1_',num2str(initial),'.txt'],'ub','-ascii')
...
for N = NList
   ...
   gs(i,2:5) = rsg_svm(algo, N, T, approx_samples, x_1, gamma, P_R, eta, lambda, training_u, training_v, test_u, test_v);
    i = i+1;
end
disp(gs(:,[2,4]));
 disp('--------------------------------End GS----------------------------------');
...
plot(NList,gs(:,2));
save(['svm_bank_gs_step_',num2str(step),'_x1_',num2str(initial),'.txt'],'gs','-ascii')
...
for N = NList
    ...
    spsa(i,2:5) = rsg_svm(algo, N, T, approx_samples, x_1, gamma, P_R, eta, lambda, training_u, training_v, test_u, test_v);
    i = i+1;
end
disp(spsa(:,[2,4]));
disp('--------------------------------End 1SPSA----------------------------------');
...
plot(NList,spsa(:,2));
save(['svm_bank_spsa_step_',num2str(step),'_x1_',num2str(initial),'.txt'],'spsa','-ascii')
...
for N = NList
    ...
    rdsa_u(i,2:5) = rsg_svm(algo, N, T, approx_samples, x_1, gamma, P_R, eta, lambda, training_u, training_v, test_u, test_v);
    i = i+1;
end
disp(rdsa_u(:,[2,4]));
...
plot(NList,rdsa_u(:,2));
save(['svm_bank_rdsa_u_step_',num2str(step),'_x1_',num2str(initial),'.txt'],'rdsa_u','-ascii')
...
for N = NList+1
    ...
    rdsa_ab(i,2:5) = rsg_svm(algo, N, T, approx_samples, x_1, gamma, P_R, eta, lambda, training_u, training_v, test_u, test_v);
    i = i+1;
end
 disp(rdsa_ab(:,[2,4]));
...
save(['svm_bank_rdsa_ab_step_',num2str(step),'_x1_',num2str(initial),'.txt'],'rdsa_ab','-ascii')
...
for N = NList
    ...
    rdsa_perm(i,2:5) = rsg_svm(algo, N, T, approx_samples, x_1, gamma, P_R, eta, lambda, training_u, training_v, test_u, test_v);
    i = i+1;
end
 disp(rdsa_perm(:,[2,4]));
 disp('--------------------------------End 1RDSA_Perm_DP----------------------------------');
plot(NList,rdsa_perm(:,2));
save(['svm_bank_rdsa_perm_step_',num2str(step),'_x1_',num2str(initial),'.txt'],'rdsa_perm','-ascii')
...
for N = NList
    ...
    rdsa_kw(i,2:5) = rsg_svm(algo, N, T, approx_samples, x_1, gamma, P_R, eta, lambda, training_u, training_v, test_u, test_v);
    i = i+1;
end
 disp(rdsa_kw(:,[2,4]));
 disp('--------------------------------End 1RDSA_KW_DP----------------------------------');
plot(NList,rdsa_kw(:,2));
save(['svm_bank_rdsa_kw_step_',num2str(step),'_x1_',num2str(initial),'.txt'],'rdsa_kw','-ascii')
...
%     P_R = gamma./sum(gamma);
%     
%     rdsa_lex(i,2:5) = rsg_svm(algo, N, T, approx_samples, x_1, gamma, P_R, eta, lambda, training_u, training_v, test_u, test_v);
%     i = i+1;
% end
%  disp(rdsa_lex(:,[2,4]));
%  disp('--------------------------------End 1RDSA_Lex_DP----------------------------------');
% plot(NList,rdsa_lex(:,2));
% save(['svm_bank_rdsa_lex_step_',num2str(step),'_x1_',num2str(initial),'.txt'],'rdsa_kw','-ascii')
...
save(['svm_bank_step_',num2str(step),'_x1_',num2str(initial)]);
