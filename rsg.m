...
function [all] = rsg(d, type, algo, N, T, Q, x_1, x_star, gamma, P_R, eta, sigma)
...
for i = 1:T % Loop for T replications
    ...
    for k = 1:R % Loop for random R iteration
      grad_unbiased = SFO(d, x_k, sigma, type, algo, eta(k)); % Estimate gradient
      x_k = x_k - gamma(k)*grad_unbiased; % Stochastic Gradient descent
      if type == 1
