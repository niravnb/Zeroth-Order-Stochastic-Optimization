...
function [all] = rsqn_svm(algo, N, T, Q, x_1, gamma, P_R, eta, lambda, u, v, test_u, test_v)
...
for i = 1:T % loop for T replications
    ...
    for k = 1:R % Loop for random R iteration
      ...
      Hhat = SVM( x_k, u(randi([1 size(u,1)]),:)', v(randi([1 size(u,1)])), lambda, eta(k), algo, 2 ); % Estimate Hessian
...
%       x_k = x_k - gamma(k)*pinv(H_k)*grad; % Stochastic quasi-Newton descent
